...
func (c joinGamePartyService) JoinGameParty(ctx context.Context, requestData *models.JoinGamePartyRequestData) error {
	...
	err := c.mongoDAO.UpdatePlayerDecisionForGameParty(ctx, requestData.PartyId, requestData.UserId, models.PlayerJoinedStatus)
	if err != nil {
		return err
	}

	_, err = c.mongoDAO.UpdateUserStatus(ctx, requestData.UserId, models.UserStatusInGame)
	if err != nil {
		return err
	}
